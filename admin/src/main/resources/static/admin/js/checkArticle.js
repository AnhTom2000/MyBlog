...
$('#table').bootstrapTable({
    url: '/admin/article/findAllUnChecked',
    method: 'GET',
    ...
    columns: [
        ...
        },
        {
            ...
            formatter: function (value, row, index) {
                ...
                // alert(sid_code);
                return '<a href="#editProject" data-toggle="modal" data-target="#look"  title="查看">' +
                    '<i class="zi zi_eye" zico="眼睛"></i>' +
                    '</a>' + '<a href="#editProject" onclick="checked(this)" id="' + row.a_id + '"  title="通過審核">' +
                    '<i class="zi zi_check" ></i>' +
                    '</a>' +
                    '<a href="javascript:void(0)"  onclick="deletes(this)" id="' + row.a_id + '" title="删除">' +
...
function checked(btn) {
    $.ajax({
        url: '/admin/article/checkedArticle',
        type: 'POST',
        cache: false,
        dataType: 'json',
        data: {
            'articleId': $(btn).attr('id')
        },
        success: function (data) {
            if (data.code == 404) {
                toastr.error("请先登陆");
