    ...
    columns: [
        ...
        ,
        {
            ...
            formatter: function (value, row, index) {
                ...
                questionTitle = row.questionContent;
                return '<a href="javascript: void(0)" onclick="checked(this)"' +
                    ' id="' + row.questionId + '"  style=""  title="通過審核">' +
                    '<i class="zi zi_check" ></i>' +
                    '</a>' +
                    '<a href="javascript: void(0)" role="button" title="填写回复" data-toggle="modal" data-target="#exampleModal"  onclick="load(this) " id="' + row.questionId + '" > ' +
                    '<i class="zi zi_msgchat"></i>' +
                    '</a>'
                    + '<a href="javascript:void(0)" onclick="deletes(this)" ' +
                    'id="' + row.questionId + '" title="删除">' +
...
function checked(btn) {
    $.ajax({
        url: '/admin/question/checkedQuestion',
        type: 'POST',
        cache: false,
        dataType: 'json',
        data: {
            'questionId': $(btn).attr('id')
        },
        success: function (data) {
            if (data.code == 404) {
                toastr.error("请先登陆");
                ...
                toastr.success("通过审核成功");
                $('#table').bootstrapTable('refresh');
            } else if (data.code == 401) {
                toastr.warning('该反馈已审核');
            }
        }, error: function () {
...
$('#table').on('check.bs.table', function (row, $element) {
    selects.push($element.questionId);
    console.log(selects);
});
// 不选中时移除元素
$('#table').on('uncheck.bs.table', function (row, $element) {
    selects.splice($.inArray($element.questionId, selects), 1);
    console.log(selects);
});
//全选时获得所有元素
...
$('#reply').click(function () {
    var replyContent =  $('#message-text');
    if (replyContent.val() == null || replyContent.val().length < 1 ) {
        toastr.error('请写回复');
        return false;
    }if( userId == null){
        toastr.error('请选择用户');
        ...
        $.ajax({
            url: '/admin/question/replyQuestion',
            type: 'POST',
            cache: false,
            dataType: 'json',
            data: {
...
function load(btn) {
    $('#recipient-name').val(+'@'+name+'   :'+questionTitle);
    replyId = $(btn).attr('id');
}
